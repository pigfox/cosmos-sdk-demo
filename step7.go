...
func step7(validatorAddress string, pubkeyJSON string, chainID string) {
	fmt.Println("Step 7: Create the genesis file")
...
func getGenesisJSON(gp GenesisParams) string {
	...
	if matched {
		fmt.Println("Valid PubKey JSON format.")
	}
      ...
      "accounts": [
        {
          ...
          "address": "` + gp.address + `",
          "pub_key":` + `"{"@type":"` + data.Type + `","key":"` + data.Key + `"}"` + `,
          "account_number": "0",
          "sequence": "0"
        }
      ]
